...
export class TableHelper {
  ...
  private getState(
    ...
    if (_gsheet === undefined) {
      throw new Error(`No such sheet ${sheetTitle}`);
    }
    let fail = () => {
      throw new Error(`Incomplete sheet ${sheetTitle}`);
    };
    ...
    if (_gtable === undefined) {
      throw new Error(`No such table ${sheetTitle}.${tableName}.`);
    }
    fail = () => {
      throw new Error(`Incomplete table ${sheetTitle}.${tableName}`);
    };
